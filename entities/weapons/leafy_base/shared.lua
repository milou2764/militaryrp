...
local HitSound = {"sweps/Hitmarker Sound.wav"}
...
function SWEP:Deploy()
    ...
    self:SetNWInt("Reloading", CurTime() + self:SequenceDuration())
...
function SWEP:Think()
    ...
    if self.IronSights == true and ply:KeyDown(IN_ATTACK2) and ply:WaterLevel() ~= 3 and sprintshit == false and wep:GetNWInt("Reloading") < CurTime() and not self:NearWall() then
        wep:SetNWBool("Ironsights", true)
        self.Primary.Cone = self.IronCone

        if self.SniperScope == true and SERVER and self:GetNWBool("FuckDaModel") == true then
    ...
    if SERVER and ply:KeyReleased(IN_ATTACK2) and self.SniperScope == true then
    ...
    if self.CSGOstyle == true then
        self.Primary.Cone = (self.DefaultCone + self:GetOwner():GetVelocity():Length() / 100) * self.CSGOMultiplier
    end

    if self.Hitmarker == true and self.MarkerOpacity >= 0 then
...
--Если надо что-то приписать к выстрелу, но не нужен весь код
function SWEP:PrimaryCall()
end
...
function SWEP:PrimaryAttack()
    ...
    if not ply:IsNPC() and (self.Primary.Recoil > 1) and ((game.SinglePlayer() and SERVER) or (not game.SinglePlayer() and CLIENT and IsFirstTimePredicted())) then
        local shotang = self:GetOwner():EyeAngles()
...
function SWEP:InspectThink()
    ...
    if CLIENT then
        if inpat.IsKeyDown(KEY_G) and wep:GetNWInt("Reloading") < CurTime() and not ply:KeyDown(IN_ATTACK) and not ply:KeyDown(IN_ATTACK2) and wep:GetNWBool("SprintShit") == false and animtime < maxanim then
            --bultst2 = Lerp(FT*2, bultst2, -35)
            if nxt < CurTime() then
                wep:SetNWInt("AnimTime", animtime + 1)
                nxt = CurTime() + ftmul
            end
        elseif ply:KeyDown(IN_ATTACK) or self:GetOwner():KeyDown(IN_ATTACK2) then
...
function SWEP:DoBones()
    ...
    for i = 0, vm:GetBoneCount() do
        ...
        if self.ViewModelBoneMods[animtime] and self.ViewModelBoneMods[bonename] then
            ...
            elseif not self.ViewModelBoneMods[animtime][bonename] then
...
function SWEP:CalcView(ply, origin, angles, fov)
    ...
    local sprintshit = Angle(0, 0, math.sin(CurTime() / 0.1) * self.SprintMul)
    self:ViewCalc()
    --[[if self:GetNWBool("Reloading") == true then
		if nxtfraem < CurTime() then
		nxtfraem = CurTime() + 1
			if angtime < maxframe then 
				self:SetNetworkedInt( "AngFrame", angtime + 1 )			
			end
		else
		//self:SetNetworkedInt( "AngFrame", 0 )
		end
    ...
    elseif self.SniperScope == true then
        --if self:GetNetworkedBool( "FuckDaModel" ) == false then
...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    --gunlagang = LerpAngle( FT*lagspeed, gunlagang, self:GetOwner():EyeAngles())
    if self.BlackMesaSprint == false then
        ...
        if ply:KeyDown(IN_SPEED) and not ply:KeyDown(IN_ATTACK) and wep:GetNextPrimaryFire() + 1 < CurTime() and ply:GetVelocity():Length() > 150 and ply:WaterLevel() < 1 and self.Sprint == true then
            SprintMul = Lerp(FT * 4, SprintMul, 1)
            wep:SetNWBool("SprintShit", true)
    ...
    --and wep:GetNWInt("Reloading") < CurTime()
    if self:NearWall() and self.NearWallType ~= 0 then
        nearwallang = math.Clamp(Lerp(FT * 5, nearwallang, (self.MaxDist - ply:GetShootPos():Distance(ply:GetEyeTrace().HitPos)) / self.MaxDist), 0, 1) --Lerp(FT*4, nearwallang, 1)
        SprintMul = Lerp(FT * 4, SprintMul, 0)
    else
...
function SWEP:AdjustMouseSensitivity()
    if self:GetNWBool("FuckDaModel", true) then
        return 0.2
    else
        return 1
    end
...
function SWEP:SecondaryAttack()
    ...
    if self:SetNWBool("Ironsights", true) then return end
    if self:GetNWInt("Reloading") > CurTime() then return end
...
function SWEP:Reload()
    ...
    if self:GetOwner():IsPlayer() and not (self:Clip1() < self.Primary.ClipSize and self:GetOwner():GetAmmoCount(self.Primary.Ammo) > 0) then return end
    self:DefaultReload(ACT_VM_RELOAD)

    --self:SetNWBool("Reloading", true)
    if self:GetOwner():IsPlayer() then
        local waitdammit = (self:GetOwner():GetViewModel():SequenceDuration())
        self:SetNWInt("Reloading", CurTime() + (waitdammit + .1))
        ...
        self:ReloadFunc()
        self:GetOwner():EmitSound(self.ReloadSound)
...
function SWEP:ShootBullet(CurrentDamage, CurrentRecoil, NumberofShots, CurrentCone, NearWallShit, trace)
    --Still modified shit from m9k lelele
    ...
    if self:GetOwner():IsPlayer() then
        vectorshit = self:GetOwner():EyeAngles() + self:GetOwner():GetViewPunchAngles() --self:GetOwner():GetViewModel():GetAngles()--
    else
        vectorshit = self:GetOwner():GetAimVector():Angle()
        CurrentDamage = CurrentDamage * 0.5
    end

    local posshit = self:GetOwner():GetShootPos()
    ...
    bullet.Src = posshit
    bullet.Dir = self:GetOwner():GetAimVector() + (vectorshit:Forward()) --* NearWallShit
...
function SWEP:ShootBulletInfo(trace)
    ...
    if self:NearWall() and self.NearWallType == 1 then
        NearWallShit = (math.Clamp((self.MaxDist - self:GetOwner():GetShootPos():Distance(self:GetOwner():GetEyeTrace().HitPos)) / (self.MaxDist / 1.5), 0, 1))
    else
...
function SWEP:HitCallback(attacker, tr, dmginfo)
    ...
    if ((tr.Entity:IsNPC()) or (tr.Entity:IsPlayer())) then
        if self.HealingBullets == true then
            tr.Entity:SetHealth(math.min(tr.Entity:GetMaxHealth(), tr.Entity:Health() + self.HealAmount))
            tr.Entity:EmitSound("items/medshot4.wav")
        end

        if self.Hitmarker == true then
            self.MarkerOpacity = 225
            local random = math.random(1, #HitSound)
            self:GetOwner():EmitSound(HitSound[random])
        end
...
--Функция для худа, если нужно оставить базовый код
...
function SWEP:DrawHUD()
    ...
    if self.CustomCrosshair == true and not self:GetNWBool("Ironsights", false) and GetConVarNumber("hg_crosshair") == 1 then
    ...
    if self.Hitmarker == true then
        ...
        srface.SetDrawColor(255, 255, 255, self:GetNWInt("Hitmarker", 0))
...
function SWEP:RenderSomeShit()
    tstdata.angles = self:GetOwner():GetAngles() + self:GetOwner():GetViewPunchAngles()
    tstdata.origin = self:GetOwner():GetShootPos()
    tstdata.x = 0
    tstdata.y = 0
    tstdata.w = ScrW()
    tstdata.h = ScrH()
...
function SWEP:NearWall()
    if self:GetOwner():IsNPC() or not self:GetOwner():IsValid() then return end

    return self:GetOwner():GetShootPos():Distance(self:GetOwner():GetEyeTrace().HitPos) <= self.MaxDist and not self:GetOwner():GetEyeTrace().Entity:IsPlayer() and not self:GetOwner():GetEyeTrace().Entity:IsNPC()
end

function SWEP:TraceCollider(addforward, addright, addup)
    local angles = self:GetOwner():EyeAngles()
    local collider = {}
    collider.start = self:GetOwner():EyePos()
    ...
    collider.filter = self:GetOwner()
    local trace = util.TraceLine(collider)

    return self:GetOwner():GetShootPos():Distance(trace.HitPos)
end
...
function SWEP:Initialize()
    ...
    if SERVER then
        self:GetOwner():ShouldDropWeapon(true)
        ...
        if self:GetOwner():IsNPC() then
            ...
            self:GetOwner():SetKeyValue("spawnflags", "155") -- Long Visibility/Shoot
            self:GetOwner():SetKeyValue("FireRate", "15")
            self:GetOwner():SetCurrentWeaponProficiency(WEAPON_PROFICIENCY_VERY_GOOD)
        end
    end
...
function SWEP:SubMatFunc()
    local vm = self:GetOwner():GetViewModel() --Get view model
...
if CLIENT then
    --[[function SWEP:PostDrawViewModel( vm, weapon ) 
		if self:GetNetworkedBool( "FuckDaModel" ) then return false end
	end]]
	...
	local mins = self:GetOwner():OBBMins()
	local maxs = self:GetOwner():OBBMaxs()
	local len = 6000
	
	local postst = self:GetOwner():GetViewModel():GetAttachment("1").Pos
	local angtst = self:GetOwner():GetViewModel():GetForward()
	...
	local tracetst = util.TraceHull( {
		...
		filter = ent
            ...
            local pos = self:GetOwner():GetViewModel():GetAttachment("1").Pos --vm:GetPos()
            local ang = self:GetOwner():GetViewModel():GetAttachment("1").Ang --GetAngles()--vm:GetAngles()
    ...
    function SWEP:DrawWorldModel()
        ...
        if self:GetOwner():IsValid() and self.LaserSight == 1 then
