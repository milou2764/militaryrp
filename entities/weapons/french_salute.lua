...
SWEP.Slot = 4
SWEP.PrintName = "Salut Militaire"
...
if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("vgui/inventory/french_salute")
end
...
function SWEP:DoBones()
    ...
    if IsValid(ply) then
        ...
        self.ViewModelBoneMods["ValveBiped.Bip01_R_UpperArm"].pos = Vector(0, -14 + (33 * ang1), -24) --zoom inclinaison hauteur gauche-droite
    end
end
...
function SWEP:SecondThink()
    ...
    if IsValid(ply) and SERVER then
        ...
        if bone then
            ply:ManipulateBoneAngles(bone, Angle(100 * ang1, -90 * ang1, -70 * ang1))
        end

        bone = ply:LookupBone("ValveBiped.Bip01_R_Forearm")

        if bone then
            ply:ManipulateBoneAngles(bone, Angle(30 * ang1, -130 * ang1, -50 * ang1))
        end
    end
end
