...
local function removeSelectedContainer(panel, gearPanel, target)
...
local function LoadInventory(parentPanel, inventoryTab, owner, context, startingIndex, yPos, max)
    local i,j = 0,0
    for k = 0,max-1 do
        ...
        local slotPanel = ent:createSlotPanel(parentPanel, owner, 25 + j * 125, yPos + i * 125, 100, 100, context, MRPCategory)
...
function MRP.createDropZone()
    ...
    function MRP.dropZone:MakeDroppable(name, allowCopy)
        self:Receiver(name, function(dest, panels, bDoDrop, Command, x, y)
            if bDoDrop then
...
function MRP.OpenChest(ent)
    ...
    MRP.chestPanel.Paint = function( self, w, h )
...
function MRP.OpenRagdollInvPanel(target)
    ...
    if target:GetNWInt("PrimaryWep") > 1 then
        ...
        PrimaryWep.progressBar.getFraction = function() return target:GetNWInt("PrimaryWepAmmo") / MRP.getMRPEnt(target:GetNWInt("PrimaryWep")).clipSize end
    end
    if target:GetNWInt("SecondaryWep") > 1 then
        ...
        SecondaryWep.progressBar.getFraction = function() return target:GetNWInt("SecondaryWepAmmo") / MRP.getMRPEnt(target:GetNWInt("SecondaryWep")).clipSize end
    end
    if target:GetNWInt("RocketLauncher") > 1 then
        ...
        RocketLauncher.progressBar.getFraction = function() return target:GetNWInt("RocketLauncherAmmo") / MRP.getMRPEnt(target:GetNWInt("RocketLauncher")).clipSize end
    end

    Rucksack.DoRightClick = function(self)
        removeSelectedContainer(MRP.ragdollInvPanel, self, target)
...
function MRP.OpenPlyInvPanel(ply)
    ...
    if ply:Has("PrimaryWep") then
        ...
        PrimaryWep.progressBar:SetX(340)
        PrimaryWep.progressBar:SetSize(10, 100)
        local swep = ply:GetWeapon(MRP.getMRPEnt(ply:GetNWInt("PrimaryWep")).wepClass)
        PrimaryWep.progressBar.getFraction = function() if IsValid(swep) then return swep:Clip1() / swep.Primary.ClipSize end return 1 end
    end
    if ply:Has("SecondaryWep") then
        ...
        SecondaryWep.progressBar:SetX(215)
        SecondaryWep.progressBar:SetSize(10, 100)
        local sswep = ply:GetWeapon(MRP.getMRPEnt(ply:GetNWInt("SecondaryWep")).wepClass)
        SecondaryWep.progressBar.getFraction = function() if IsValid(sswep) then return sswep:Clip1() / sswep.Primary.ClipSize end return 1 end
    end
    if ply:Has("RocketLauncher") then
        ...
        RocketLauncher.progressBar:SetX(590)
        RocketLauncher.progressBar:SetSize(10, 100)
        local rlauncher = ply:GetWeapon(MRP.getMRPEnt(ply:GetNWInt("RocketLauncher")).wepClass)
        RocketLauncher.progressBar.getFraction = function() if IsValid(rlauncher) then return rlauncher:Clip1() / rlauncher.Primary.ClipSize end return 1 end
    end
    ...
    Rucksack.DoRightClick = function(self)
        removeSelectedContainer(MRP.plyInvPanel, self, ply)
        self.gear:dropFromInventoryPanel(self)
    end
    Vest.DoRightClick = function(self)
        removeSelectedContainer(MRP.plyInvPanel, self, ply)
...
function CreateInventoryPanel(target, context)
    ...
    function panel.ReloadSelectedContainer(startingIndex)
        ...
        LoadInventory(panel, panel.InvSlot, target, context, startingIndex, 830, panel.SelectedContainer.gear.Capacity)
    end
    ...
    local headpos = panel.pmodel.Entity:GetBonePosition(panel.pmodel.Entity:LookupBone("ValveBiped.Bip01_Head1"))
    ...
    panel.NVGs = baseclass.Get("mrp_nvgs"):createSlotPanel(panel, target, 375, 25, 100, 100, context, "NVGs").entPanel or {} -- INDEX 1
    ...
    panel.Helmet = baseclass.Get("mrp_base_helmet"):createSlotPanel(panel, target, 500, 25, 100, 100, context, "Helmet").entPanel or {} -- INDEX 2
    if IsValid(panel.Helmet) then
        panel.pmodel.Entity:AddGear(panel.Helmet)
    end

    panel.Gasmask = baseclass.Get("mrp_base_gear"):createSlotPanel(panel, target, 500, 150, 100, 100, context, "Gasmask").entPanel or {} -- INDEX 3
    if IsValid(panel.Gasmask) then
        panel.pmodel.Entity:AddGear(panel.Gasmask)
        if target:GetNWBool("GasmaskOn") then panel.Gasmask.model:SetModel("models/gmod4phun/props/gasmask.mdl") end
    end

    panel.Rucksack = baseclass.Get("mrp_base_rucksack"):createSlotPanel(panel, target, 375, 275, 100, 100, context, "Rucksack").entPanel or {} -- INDEX 4
    if IsValid(panel.Rucksack) then
        panel.Rucksack.startingIndex = 11
        panel.pmodel.Entity:AddGear(panel.Rucksack)
    end

    panel.Vest = baseclass.Get("mrp_base_vest"):createSlotPanel(panel, target, 500, 275, 100, 100, context, "Vest").entPanel or {} -- INDEX 5
    ...
    panel.Uniform.GetParent = function(self) return panel.pmodel end
    ...
    local targetPrimaryWep = MRP.getMRPEnt(target:GetNWInt("PrimaryWep"))
    panel.PrimaryWep = targetPrimaryWep:createSlotPanel(panel, target, 25, 580, 350, 100, context, "PrimaryWep", 100, 128).entPanel

    panel.SecondaryWep = MRP.getMRPEnt(target:GetNWInt("SecondaryWep")):createSlotPanel(panel, target, 25 + 350 + 25, 580, 225, 100, context, "SecondaryWep", 80).entPanel

    panel.RocketLauncher = MRP.getMRPEnt(target:GetNWInt("RocketLauncher")):createSlotPanel(panel, target, 25, 705, 600, 100, context, "RocketLauncher", 170, 256).entPanel
...
hook.Add("Tick", "InventoryOpening", function()
    if input.IsKeyDown(MRP.keybinds.inventory) and player_manager.GetPlayerClass(LocalPlayer()) ~= "player_spectator" and keyReleased then
        keyReleased = false
        if not MRP.plyInvPanel or not MRP.plyInvPanel:IsValid() and not vgui.CursorVisible() then
            MRP.createDropZone()
...
net.Receive("MRPPlayerTakeOnGasmask", function()
    ...
    if IsValid(MRP.plyInvPanel) then MRP.plyInvPanel.Gasmask.model:SetModel("models/gmod4phun/props/gasmask.mdl") end
end)
net.Receive("MRPPlayerTakeOffGasmask", function()
    ...
    if IsValid(MRP.plyInvPanel) then MRP.plyInvPanel.Gasmask.model:SetModel("models/yukon/props/conscripts/maskbag.mdl") end
end)
...
hook.Add("NetworkEntityCreated", "MRPNetworkEntityCreated", function(ent)
    ...
    if owner:IsPlayer() and ent:GetClass() == "prop_ragdoll" then
        ...
        for k,gear in pairs(MRP.mountedGear[userid]) do
            ...
            MRP.mountedGear[ragdollid][k] = ClientsideModel(gear:GetModel(), RENDERGROUP_OPAQUE)
